...
void World::Display(sf::Sprite* tile, sf::RenderWindow &app)
{
     for(int j = 0 ; j < LEVEL_HEIGHT ; j++)
        for(int i = 0 ; i < LEVEL_WIDTH ; i++)
        {
            ...
            if(m_world[i][j].tile != NOTHING_WORLD)
            {
...
bool World::SquareIsEmptyIndex(int indexX, int indexY)
{
    if(m_world[indexX][indexY].tile == QUADRILLAGE_WORLD)
        return true;
    ...
    for(int j = 0 ; j < LEVEL_HEIGHT ; j++)
    {
        for(int i = 0 ; i < LEVEL_WIDTH ; i++)
        {
            if(!SquareIsEmptyIndex(i, j))
                m_world[i][j].ID = GetIDFromIndex(i, j);
